...
async def _(Mbot,message):
    ...
    if "shorts" in link:
        try:
            ...
            if LOG_GROUP:
               ...
               await message.reply(f"400: Sorry, Unable To Find It  try another or report it  to @Rishu1286 or support chat @Ur_rishu_143 🤖  ")
               print(traceback.format_exc())
               await Mbot.send_message(LOG_GROUP, traceback.format_exc())
               
        return await message.reply("Check out @Ur_rishu_144(music)  @RishuXbot(Channel) \n Please Support Us By /donate To Maintain This Project")
    try:
  ...
  #          await message.reply(id[2])
        ...
        #    await message.reply_audio(fileLink)
            AForCopy = await message.reply_audio(fileLink,caption=f"[{id[3]}](https://youtu.be/{id[0]}) - {id[2]} Thank you for using our bot join🥰- @Ur_rishu_143",title=id[3].replace("_"," "),performer=id[2],thumb=thumnail,duration=id[4])
            if DUMP_GROUP:
        ...
        if os.path.exists(randomdir):
           rmtree(randomdir)
        await message.reply("Check out @Ur_rishu_143(music)  @RishuXbot(Channel) \n Please Support Us By /donate To Maintain This Project")
    except Exception as e:
        print(e)
        if LOG_GROUP:
